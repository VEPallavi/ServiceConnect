...
class SettingTermAndConditionViewModel : ViewModel(){
    var termAndConditionResult: MutableLiveData<JsonObject>?= null


    fun getTermAndCondition(mContext: Context): MutableLiveData<JsonObject>{
        termAndConditionResult = MutableLiveData()

        var apiService = ApiClient.getClient().create(ApiService::class.java)
        var call = apiService.termAndCondition()
        ...
        call.enqueue(object: retrofit2.Callback<JsonObject>{
            ...
            override fun onResponse(call: Call<JsonObject>, response: Response<JsonObject>) {
                ...
                if(response!= null && response.body()!= null){
                    termAndConditionResult!!.value = response.body()
                }
            }
        })
